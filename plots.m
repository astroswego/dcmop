...
for ii = 2:num_p2
    x(:,ii+num_p1)   = sin(0.71 * ii/2*ust);
    x(:,ii+num_p1+1) = cos(0.71 * ii/2*ust);
end
...
for ii = 2:2:num_p2
    wm = wm + w(ii+num_p1)   * sin(0.71 * ii/2*t) ...
            + w(ii+num_p1+1) * cos(0.71 * ii/2*t);
end
...
for ii = 2:2:num_p2
    lm = lm + w(ii+num_p1)   * sin(0.71*ii/2*t) ...
            + w(ii+num_p1+1) * cos(0.71*ii/2*t);
end
...
figure;
plot(t, m, 'color', [0 .5 0], 'LineWidth', 1.5);
xlabel('t');
ylabel('m(t)');
set(gca,'xtick',[]);
set(gca,'xticklabel',[]);
...
errorbar(t, m, e, '.', 'color', [0 .5 0], 'MarkerSize', 1)
...
for jj = 1:length(p)
    ...
    ph = mod(t ./ p(jj), 1);
    errorbar(ph, m, e, '.', 'color', [0 .5 0], 'MarkerSize', 1)
    set(gca, 'YDir', 'reverse')
    set(gca, 'xlim', [0 1])
    set(gca, 'ylim', [min(m)-2*max(e) max(m)+2*max(e)])
    set(gca, 'YTick', [])
...
for ii = 2:2:num_p1
    x(:,ii)   = sin(2*pi/p(1)*ii/2*t);
    x(:,ii+1) = cos(2*pi/p(1)*ii/2*t);
end
for ii = 2:num_p2
    x(:,ii+num_p1)   = sin(2*pi/p(2)*ii/2*t);
    x(:,ii+num_p1+1) = cos(2*pi/p(2)*ii/2*t);
end
...
for ii = 2:2:num_p1
    lm = lm + w(ii)   * sin(2*pi/p(1)*ii/2*ph) ...;
            + w(ii+1) * cos(2*pi/p(1)*ii/2*ph);
end
for ii = 2:2:num_p2
